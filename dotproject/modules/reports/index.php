...
$obj = new CProject();
$projects = $obj->getAllowedProjectsInRows($AppUI->user_id);                                                                                
...
if ($report_type) {
	...
	foreach ($reports as $v) {
		...
		echo '</a></td><td>' . (@$desc[1] ? "- $desc[1]" : '') . '</td></tr>';
	}
	echo '</table>';
} else {
	echo '
...
<tr>
	...
	foreach ($report_categories as $v) {
		$type = $v;
		if ($type != 'reports.class.php') {
			$desc_file = "$v.$AppUI->user_locale.txt";
			$desc = @file(DP_BASE_DIR.'/modules/reports/'.$desc_file );
	
			echo "<tr><td><a href=\"index.php?m=reports&amp;project_id=$project_id&amp;report_category=$v";
			if (isset($desc[2])) {
				echo "&" . $desc[2];
			}
			echo '">';
			echo @$desc[0] ? $desc[0] : ucfirst($v);
			echo '</a>';
			echo '</td><td>' . (@$desc[1] ? "- $desc[1]" : '') . '</td></tr>';
		}
	}
	echo '</table>';
}
